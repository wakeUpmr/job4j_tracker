...
public class StartUITest {
    ...
    @Test
    public void whenFindItemByIdTestOutputIsNotSuccessfully() {
        Output out = new StubOutput();
        Tracker tracker = new Tracker();
        Item one = tracker.add(new Item("test1"));
        int wrongId = one.getId() + 1;
        Input in = new StubInput(
                new String[] {"0", String.valueOf(wrongId), "1"}
        );
        List<UserAction> actions = Arrays.asList(
                new FindByIdAction(out),
                new ExitAction()
    ...
    public void whenFindItemByNameTestOutputIsSuccessfully() {
        ...
        Item one = tracker.add(new Item(searchName));
        Input in = new StubInput(
